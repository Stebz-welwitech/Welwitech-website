...
export default function ServiceCard({ title, desc, href, variant, delay = 0 }) {
  ...
  return (
    <a href={href} className={s.card} ref={ref} onClick={handleClick}>
      <div className={s.icon}>{icons[variant]}</div>
      <h3 className={s.title}>{title}</h3>
      <p className={s.desc}>{desc}</p>
      <span className={s.link}>En savoir plus
        <svg className={s.arrow} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
      </span>
    </a>
  )
}
